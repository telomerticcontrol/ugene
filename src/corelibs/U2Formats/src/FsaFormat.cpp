...
DNASequence *FsaFormat::loadSequence(IOAdapter *io, U2OpStatus &os) {
    if (io->isEof()) {
        return NULL;
    }
...
bool FsaFormat::loadABIObjects(SeekableBuf* fp, DNASequence &dna, DNAChromatogram &cd) {
    uint numPoints = 0;
    uint numBases = 0;
    //uint signalO;
    int no_bases = 0;
    ...
    uint fwo_ = 0;     /* base -> lane mapping */
    uint indexO = 0;   /* File offset where the index is */
    //uint baseO;    /* File offset where the bases are stored */
    uint dataCO = 0;   /* File offset where the C trace is stored */
    uint dataAO = 0;   /* File offset where the A trace is stored */
    uint dataGO = 0;   /* File offset where the G trace is stored */
    uint dataTO = 0;   /* File offset where the T trace is stored */
    uint offset = 0;   /* Generic offset */
    uint offset2 = 0;  /* Generic offset */
    uint offset3 = 0;  /* Generic offset */
    uint offset4 = 0;  /* Generic offset */
